...
gm12878_5kb_f_nl <- gm12878_5kb_f

cols <- c(grep("dist",colnames(gm12878_5kb_f_nl)))
gm12878_5kb_f_nl[,cols] <- apply(gm12878_5kb_f_nl[,cols], 2, function(x){2^x})
...
enetlst_nlns <- list(tpr <- matrix(nrow=ceiling((length(which(gm12878_5kb_f_nl$y=="Yes"))*2)*.3), 
                                 ncol=1),
                   fpr <- matrix(nrow=ceiling((length(which(gm12878_5kb_f_nl$y=="Yes"))*2)*.3), 
                                 ncol=1),
                   varimp <- matrix(nrow=dim(gm12878_5kb_f_nl)[2]-1,
                                    ncol=1))
rownames(enetlst_nlns[[3]]) <- colnames(gm12878_5kb_f_nl)[-1]
