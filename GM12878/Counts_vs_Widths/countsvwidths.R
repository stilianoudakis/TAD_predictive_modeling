...
gm12878_5kb <- gm12878_5kb[,grep("dist", names(gm12878_5kb), invert=TRUE)]
...
saveRDS(rflst,"rflst_cvw.rds")
saveRDS(rfperf,"rfperf_cvw.rds")	

####################################################################################

rflst_cvw <- readRDS("rfperf_cvw.rds")

varimp.cvw <- rowMeans(rflst_cvw[[3]])
varimp.cvw.df <- data.frame(Feature = rownames(rflst_cvw[[3]]), Importance =as.vector( varimp.cvw))
varimp.cvw.df <- varimp.cvw.df[order(varimp.cvw.df$Importance, decreasing=FALSE),]
varimp.cvw.df <- varimp.cvw.df[(dim(varimp.cvw.df)[1]-9):dim(varimp.cvw.df)[1],]
varimp.cvw.df$Feature <- factor(varimp.cvw.df$Feature,
                                     levels=varimp.cvw.df$Feature)
p.cvw <- ggplot(varimp.cvw.df, aes(x=Feature, y=Importance)) +
  xlab("Predictors") +
  ylab("Importance") +
  ...
  ggtitle("Elastic-Net")
p.cvw	
ggsave("/home/stilianoudakisc/TAD_data_analysis/model_filtering/count_vs_width/p.cvw.png")
