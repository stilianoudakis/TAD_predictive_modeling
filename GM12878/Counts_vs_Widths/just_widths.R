...
gm12878_5kb <- gm12878_5kb[,-which(names(gm12878_5kb) %in% matches)]
...
#rownames(enetlst[[4]]) <- colnames(gm12878_5kb)[-1]
...
saveRDS(rflst,"rflst_w.rds")
saveRDS(rfperf,"rfperf_w.rds")	


####################################################################################

varimp.w <- rowMeans(rflst_w[[3]])
varimp.w.df <- data.frame(Feature = rownames(rflst_w[[3]]), Importance =as.vector( varimp.w))
varimp.w.df <- varimp.w.df[order(varimp.w.df$Importance, decreasing=FALSE),]
varimp.w.df <- varimp.w.df[(dim(varimp.w.df)[1]-9):dim(varimp.w.df)[1],]
varimp.w.df$Feature <- factor(varimp.w.df$Feature,
                                     levels=varimp.w.df$Feature)
p.w <- ggplot(varimp.w.df, aes(x=Feature, y=Importance)) +
  xlab("Predictors") +
  ylab("Importance") +
  ...
  ggtitle("Elastic-Net")
p.w	
ggsave("/home/stilianoudakisc/TAD_data_analysis/model_filtering/count_vs_width/p.w.png")
