...
lassolst <- list(tpr <- matrix(nrow=ceiling((length(which(gm12878_10kb$y=="Yes"))*2)*.3), 
                              ncol=1),
				...
				coefs <- matrix(nrow=dim(gm12878_10kb)[2]-1,
									ncol=1))
rownames(lassolst[[3]]) <- colnames(gm12878_10kb)[-1]
rownames(lassolst[[4]]) <- colnames(gm12878_10kb)[-1]				 
