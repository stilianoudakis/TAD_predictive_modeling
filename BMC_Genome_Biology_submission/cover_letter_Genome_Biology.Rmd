...
- *preciseTAD* prioritizes genomic signatures associated with domain boundaries. Trained on any 3D genomic features (e.g., besides TAD/loop boundaries, regions of A/B compartment switching, frequently interacting regions (FIREs), differentially interacting chromatin regions, etc.), *preciseTAD* defines (epi)genomic signatures most predictive of those features.

- The provided models pre-trained on Arrowhead-called TAD boundaries and Peakachu-called loop boundaries can predict domain boundaries in cells without Hi-C data using only genomic annotation data.

The main findings of our work are:

- Transcription factors, spatial relationships (distance within the linear genome) between them and boundaries, random under-sampling are the critical features needed for accurate boundary predictions.

- Only four transcription factors, CTCF, RAD21, SMC3, ZNF143, are sufficient for domain boundary predictions.

- Boundaries of TADs and loops are associated with the same genome annotation signatures, suggesting similar mechanisms of their formation.
