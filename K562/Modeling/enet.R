...
enetlst <- list(tpr <- matrix(nrow=ceiling((length(which(k562_10kb_reduced$y=="Yes"))*2)*.3), 
                                  ncol=bootsamps),
                    fpr <- matrix(nrow=ceiling((length(which(k562_10kb_reduced$y=="Yes"))*2)*.3), 
                                  ncol=bootsamps),
                    varimp <- matrix(nrow=dim(k562_10kb_reduced)[2]-1,
                                     ncol=bootsamps),
					coefs <- matrix(nrow=dim(k562_10kb_reduced)[2]-1,
									ncol=bootsamps))
rownames(enetlst[[3]]) <- colnames(k562_10kb_reduced)[-1]
rownames(enetlst[[4]]) <- colnames(k562_10kb_reduced)[-1]
...
saveRDS(enetlst,"enetlst_k562.rds")
saveRDS(enetperf,"enetperf_k562.rds")
