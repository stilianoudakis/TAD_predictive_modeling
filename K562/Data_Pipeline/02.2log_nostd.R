...
enetlst_lns <- list(tpr <- matrix(nrow=ceiling((length(which(k562_10kb_f$y=="Yes"))*2)*.3), 
                                 ncol=1),
                   fpr <- matrix(nrow=ceiling((length(which(k562_10kb_f$y=="Yes"))*2)*.3), 
                                 ncol=1),
                   varimp <- matrix(nrow=dim(k562_10kb_f)[2]-1,
                                    ncol=1))
rownames(enetlst_lns[[3]]) <- colnames(k562_10kb_f)[-1]

enetperf_lns <- matrix(nrow = 17, ncol=1)
...
#Prediction vector for other performance metrics
  ...
  enetperf_lns[3,1] <- FP
  enetperf_lns[4,1] <- TP
  enetperf_lns[5,1] <- sum(confMat$table)
  enetperf_lns[6,1] <- as.vector(confMat$byClass["Sensitivity"])
  enetperf_lns[7,1] <- as.vector(confMat$byClass["Specificity"])
  enetperf_lns[8,1] <- as.vector(confMat$overall["Kappa"])
  enetperf_lns[9,1] <- as.vector(confMat$overall["Accuracy"])
  enetperf_lns[10,1] <- TP/(TP+FP)
  enetperf_lns[11,1] <- FP/(FP+TN)
  enetperf_lns[12,1] <- FN/(FN+TN)
  enetperf_lns[13,1] <- FN/(FN+TN)
  enetperf_lns[14,1] <- TN/(TN+FN)
  enetperf_lns[15,1] <- (TP*TN - FP*FN)/( sqrt( (TP+FP)*(TP+FN)*(TN+FP)*(TN+FN) ) )
  enetperf_lns[16,1] <- (2*(TP/(TP+FN))*(TP/(TP+FP)))/((TP/(TP+FN)) + ((TP/(TP+FP))))
  enetperf_lns[17,1] <- pROC::auc(pROC::roc(test$y, pred.eNetModel))
