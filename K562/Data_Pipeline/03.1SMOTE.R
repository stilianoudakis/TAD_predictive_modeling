...
# Splitting the data
set.seed(5228)
inTrainingSet <- createDataPartition(k562_10kb_f$y,p=.7,list=FALSE)
train <- k562_10kb_f[inTrainingSet,]
test <- k562_10kb_f[-inTrainingSet,]
...
enetlst_sm <- list(tpr <- matrix(nrow=length(test$y), 
                   ...
                   varimp <- matrix(nrow=dim(k562_10kb_f)[2]-1,
                                    ncol=4))
rownames(enetlst_sm[[3]]) <- colnames(k562_10kb_f)[-1]
