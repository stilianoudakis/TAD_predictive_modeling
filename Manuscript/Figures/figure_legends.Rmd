...
**Figure 9-13. Recursive Feature Elimination for 10 kb data** Beginning with the full set of distance type predictors for each cell line, models were recursively evaluated at different subsets of feature size. For convenience and computational efficiency, feature size decreasing in powers of 2 was considered. For the 10 kb data, near optimal performance was achieved for both the GM12878 (red) and K562 cell lines (blue). The maximum size for feature subsets differed between cell line due the available annotations provided in Supplementary Table 1.

Performances of a series of random forest models with different numbers of the most predictive features for each cell line was compared using AUPRC. For convenience and computational efficiency, only specific subsets of features were considered (increasing powers of 2). 

Additionally, models using the full feature space were also evaluated, hence the differing number of features between cell lines. For the 10 kb data, near optimal performance was achieved for both the GM12878 (red) and K562 cell lines (blue).
