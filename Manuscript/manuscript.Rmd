...
## Random under-sampling, distance-based predictors, and 10 kb resolutions of Hi-C data provided the optimal data level characteristics for TAD boundary prediction

Expectedly, when using data as-is, that is, with class imbalance present, the models performed poorly. Using any of the resampling techniques was found to improve model performance, with RUS narrowly out performing SMOTE re-sampling as the most optimal class balancing technique for all resolutions (Figure 3). These results suggest that simple randomly under-sampling alone can drastically improve TAD boundary prediction. <!--These results suggest that a combination of both random over- and under-sampling (SMOTE) can improve TAD boundary prediction.-->
...
![**Models built using distance-type predictors combined with random under-sampling yielded optimal predictive performance.** Line plots comparing the performances of random forest classifiers built using different combinations of data level characteristics. Performances are compared across resolution, within each predictor-type: (A) OC, (B) OP, and (C) Distance, and between each re-sampling technique: no re-sampling (None; red), random over-sampling (ROS; green), random under-sampling (RUS; blue), and synthetic minority over-sampling (SMOTE; purple). Performances were evaluated using MCCs and aggregated by averaging across each chromosome, with error bars representing 1 standard deviation away from the mean.](Figures/performances.png)
