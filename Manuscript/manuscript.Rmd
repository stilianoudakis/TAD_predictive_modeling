...
While direct overlaps represent the most straightforward measure of functional associations, they do not account for indirect relationships shown to be one of the major driving force of genome regulation [??? REFS]. The most prominent example maybe the enhancer-promoter relationships, with enhancers are frequently located hundreths or thousands bases away from their target promoters [??? REFS]. The use of distances between TAD boundaries and genomic annotations has largely been ignored. However, these distance features have the potential to offer a more accurate spatial representation of which genomic features are associated with TAD boundaries.
...
The efficacy of each re-sampling technique was compared using the F1-score, MCC, and balanced accuracy. These metrics are defined as follows:
...
Multiple random forest classification algorithms were built on domain data extracted from Hi-C data for 2 cell lines $C = \{GM12878, K562\}$, at four different resolutions $R = \{10 kb, 25 kb, 50 kb, 100 kb\}$. For each classifier, we compared three predictor types $P = \{\text{OC}, \text{OP}, \text{Distance}\}$ and 4 class balancing techniques $B = \{None, RUS, ROS, SMOTE}$. A system produced from the ensemble framework was defined as: $S = \{c, r, p, b\}$, where $c \in C, \quad R \in R, \quad p \in P,$ and $b \in B$. Letting $|X|$ denote the cardinality of each set, we evaluated $|C| \times |R| \times |P| \times |B|$ different ensemble systems, totaling $96 (= 2 \times 4 \times 3 \times 4)$ separate models. 

## IMBALANCE RATIOS REMAINED SEVERE AND CONSISTENT ACROSS RESOLUTIONS

We examined the class imbalance of the different data resolutions for each cell line. As expected, the total number of genomic bins decreased as resolution decreased. That is, at lower resolutions, the linear genome was binned into larger units, thereby creating fewer bins (Supplementary Figure 15 (A)). The imbalance ratios, however, remained relatively stable (Supplementary Figure 15 (B)). We found that there was an average IR of 0.154 (SD=0.022) and 0.137 (SD=0.0181) across resolutions for the GM12878 and K562 cell lines respectively. Ideally, a balanced dataset will have an IR close to 0.50. Here, the low imbalance ratios indicated a wide disparity between majority and minority classes (i.e. a genomic bin was much more likely to not overlap with a flanked TAD boundary than it was to overlap with one). Therefore suitable resampling techniques were needed.  

## HIGH CORRRELATIONS IMPLY COLOCALIZATION OF MANY ANNOTATIONS AROUND TAD BOUNDARIES

By isolating bins that overlapped with flanked TAD boundaries we were able to assess the correlations of genomic annotations and determine which annotations were enriched together at boundaries. To  get a hollisitic view of how some annotations tended to occur together at boundaries, we computed the pairwise correlations of the averages between count overlaps, count percents, and $log_{2}$ distances of each genomic annotation with bins that overlapped with flanked TAD boundaries. Clustered correlograms at the 10 kb resolution, for both cell lines, showed that distinct groups of transcription factors were highly correlated including CTCF, ZNF143, RAD21, and SMC3ab (Figure 7). Likewise, combinations of particular histone modifications and chromatin states were also shown to exhibit similar profiles near TAD boundaries including H3K4me1, H3K4me2, and H3K4me3, aswell as strong and weak enhancers. Furthermore, particular chromatin segmentation states, inclduing heterochromatin, repressed chromatin, poised promotors, and repetitive copy number variations, were found to be highly correlated, yet inversely correlated with many transcription factor binding sites. Similar correlations were exhibited across the remainder of resolutions, for both cell lines (Supplementary Figure X). The high correlations among many of the genomic annotations gave precedent for the use of an elastic-net regularization algorithm in order to suitably reduce the feature space while retaining possibly correlated annotations that could be predictive of TAD boundary formation. The regularization was performed predictor-type and resolution specific data after implementing each of the resampling techniques within each cell line. 

## CLASS IMBALANCE SEVERELY IMPACTS MODEL PERFORMANCE ACROSS RESOLUTIONS

We first evaluated model performances using only the overlap counts predictor types without re-sampling as these characteristics represented models with the least granularity and could act as a baseline. We measured the performances across resolutions for each cell line (Figure 7 (A)). Some notable results became clear. First, we see that the conventional AUC value was unable to accurately portray the poor performance of the models when compared to the composite metrics F1-score, MCC, and, to a lesser extent, the balanced accuracy. Inspecting the F1-score and MCC more closely, we see that some models at lower resolutions (for either cell line) were unable to correctly predict any bins that overlap with flanked TAD boundaries in the test data, as indicated by the missing bars.

## DATA RE-SAMPLING IMPROVES MODEL PERFORMANCE

Comparisons of the effects of the various resampling techniques were then made across resolutions, for each cell line. Results showed that performances drastically improved among the composite metrics (Figure 7 (B-D)). Models implemented on randomly under-sampled and SMOTE-sampled data performed similarly, while performances on randomly over-sampled data lagged behind, yet still outperformed models with no data re-sampling. Overall, there appeared the be a downward trend in model performance as resolutions decreased, across both cell lines.

## USING DISTANCE-TYPE PREDICTORS FURTHER IMPROVED MODEL PERFORMANCES

After illustrating the improvement in model performance using re-sampled data for only the overlap counts predictor types, we then directly compared the performances of each re-sampling techniques on the other two predictor types (overlap percents and distances). Immediately, results showed that models with distance-type predictors outperformed models with other predictor types (Figure 8 (A-D)). This results was consistent across resolution and cell line. As before, we are able to gain much more insight into the performances of the models by using composite metrics as opposed to simply the AUC, in cases of class imbalance. Likewise, performances were shown to continue to degrade as resolutions decreased.

## MODELS IMPLEMENTED ON SMOTE RE-SAMPLED DATA WITH DISTANCE-TYPE PREDICTORS PERFORMED BEST AMOUNG ALL ENSEMBLE SYSTEMS

Of the re-sampling techniques considered, it was found that SMOTE re-sampled datasets, implemented on distance-type predictors, yielded the highest model performances across each of the 3 composite metrics (Figure 9). These results were consistent across resolution and cell line. Although RUS performed only marginally worse than SMOTE, there was a steap drop off in performances for ROS and no re-sampling, across both resolution and cell line. These results imply that a best performing model, one that is able to best discriminate between genomic bins that are near versus far away from TAD boundaries, is one that incorporates both random under- and over-sampling, as SMOTE is known to do.

## MODELS IMPLEMENTED ON SMOTE RESAMPLED DATA PERFORMED BEST

Of the, resampling techniques considered, it was found that SMOTE resampled datasets, at the 10 kb resoltion, yielded the highest model performances across each metric. This result was, for the most part, consistent for the other data resolutions. Performances of randomly under-sampled datasets were found to be comparable to, if not better than (at some isolated resolutions; see supplementary), SMOTE. The steap drop off in performance for randomly over-sampled and no resampled datasets was most evident when assessing F1-scores and balanced accuracies. Measures of MCC were found to increase more steadily toward models implemented on SMOTE resampled datasets. These results imply that a best performing model, one that is able to best discriminate between genomic bins that are near versus far away from TAD boundaries, is one that incorporates both random under- and over-sampling, as SMOTE is known to do.

Furthermore, we found that model performances tended to decline as data resolution decreased. Random forest models implemented on 10 kb, SMOTE resampled, data resolutions, with distance-type predictors were found to perform best according to MCC and balanced accuracy. There was a slight improvement in F1-Score for the 25 kb resolution on the GM12878 cell line. Performances from the K562 cell line degraded much faster as resolution decreased, while the worst performing models for the GM12878 cell line were at the 50 kb resolution, followed by a slight improvement in performance at the 100 kb resolution. 
