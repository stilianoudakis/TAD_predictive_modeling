...
**Methods.** We proposed a novel ensemble framework that systematically addresses those data irregularities and characteristics. Multiple random forest classification models, in combination with variations in cell line, data resolution, feature engineering, and re-sampling techniques were performed and compared. A number of performance metrics were assessed, including [??? update when ready].
...
The efficacy of each re-sampling technique was compared using composite metrics inclusing F1-score and MCC. These metrics are defined as follows:
...
## CLASS IMBALANCE SEVERELY IMPACTS PERFORMANCE AMONG MODELS WITH EITHER OVERLAP COUNTS OR PERCENTS PREDICTOR TYPES 

We first evaluated model performances for the GM12878 cell line, using only the overlap counts predictor types, without re-sampling, as these characteristics represented models with the least granularity and could act as a baseline. We measured the performances across resolutions, and found that a conventional measure such as AUC artificially inflated model performances (Figure 5A (I)). In evaluating the composite metrics like F1-Score and MCC, we found more reasonable depictions of poor performance, given such high levels of class imbalance (Figure 5A (II-IV)). At the 50 kb and 100 kb resolution, for overlap counts, the F1-Score and MCC could not be determined given by the model's complete inability to correctly classify any genomic bins that overlapped with flanked TAD boundaries in the test data. When including overlap percents perdictor types in the models, we found that AUC values were comparable to OC models, across resolution. However, when considering both the F1-Score and MCC, we found that OP models performed worse than OC models. Similarly, at the 50 kb and 100 kb resolutions, models built on OP were unable produce F1-Scores or MCC values. Similar results were found for the K562 cell line (Supplementary Figure 10A (I-IV)).

## DATA RE-SAMPLING IMPROVES PERFORMANCE AMONG MODELS WITH OVERLAP COUNTS PREDICTOR TYPES 

Comparisons of the effects of the various resampling techniques were then made for models using overlap counts predictor types, across resolutions. Simply considering AUC did not depict any notable improvement in performances, regardless of the re-sampling technique (Figure 5B-5D (I)). However, when considering the other composite metrics we found that there was a substantial increase in F1-Score and MCC in each of the 3 resampling techniques. For overlap counts predictors, both RUS and SMOTE resampled datasets were found to yield consistently better results than no resampling and ROS.

Results showed that performances drastically improved among the composite metrics (Figure 8 (B-D)). Models implemented on randomly under-sampled and SMOTE-sampled data performed similarly, while performances on randomly over-sampled data lagged behind, yet still outperformed models with no data re-sampling. Overall, there appeared the be a downward trend in model performance as resolutions decreased, across both cell lines.
