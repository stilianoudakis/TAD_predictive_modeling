...
We examined the class imbalance of the different data resolutions for each cell line. As expected, the total number of genomic bins decreased as resolution decreased. That is, at lower resolutions, the linear genome was binned into larger units, thereby creating fewer bins (Figure 4 (A)). The imbalance ratios, however, remained relatively stable (Figure 4 (B)). We found that there was an average IR of 0.154 (SD=0.022) and 0.137 (SD=0.0181) across resolutions for the GM12878 and K562 cell lines respectively. Ideally, a balanced dataset will have an IR close to 0.50. Here, the low imbalance ratios indicated a wide disparity between majority and minority classes (i.e. a genomic bin was much more likely to not overlap with a flanked TAD boundary than it was to overlap with one). Therefore suitable resampling techniques were needed.  

## HIGH CORRRELATIONS IMPLY COLOCALIZATION OF MANY ANNOTATIONS AROUND TAD BOUNDARIES

By isolating bins that overlapped with flanked TAD boundaries and, in turn, counting the instances in which annotations overlapped with said bins (using the overlap counts discussed above) we were able to assess the correlations of genomic annotations and determine which annotations were enriched together at boundaries.

Clustered correlograms for known colocalized annotations showed that distinct groups of transcription factors were highly correlated including CTCF, ZNF143, RAD21, and SMC3ab (Supplementary Figures 2-9). Likewise, combinations of particular histone modifications and chromatin states were also shown to exhibit similar profiles near TAD boundaries including H3K4me1, H3K4me2, and H3K4me3, aswell as strong and weak enhancers. Furthermore, particular chromatin segmentation states, inclduing heterochromatin, repressed chromatin, poised promotors, and repetitive copy number variations, were found to be highly correlated, yet inversely correlated with many transcription factor binding sites. These results were consistent across resolution and cell line. The high correlations among many of the genomic annotations gave precedent for the use of an elastic-net regularization algorithm in order to suitably reduce the feature space while retaining possibly correlated annotations that could be predictive of TAD boundary formation. 
