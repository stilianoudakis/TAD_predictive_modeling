...
We assessed the collinearity among the genomic annotations used for the feature space of subsequent models by isolating the genomic bins that overlapped with flanked TAD boundaries. We then vectorized the number of instances when a region defined by a genomic annotation overlapped with the isolated bins. It was found that the correlations among these vectors were very high for many genomic annotations. The correlations were visualized using correlograms for each data resolution and cell line (Supplementary Figure 2). The high correlations imply that many genomic annotations tended to colocalize near each other at the boarders of domains. This was present across both resolution and cell line. Specifically, we found that the CTCF, Rad21, Smc3ab, and Znf143 transcription factor binding sites were highly colocalized near TAD boundaries. [??? We need to quantify and show this, and the following colocalizations] Likewise, many histone modifications also tended to appear near TAD boundaries in conjunction with one another including H3K4me1, H3K4me2, H3K4me3, H3K9ac, and H3K27ac. Furthermore, particular chromatin segmentation states, inclduing heterochromatin, repressed chromatin, poised promotors, and repetitive copy number variations, were found to be highly correlated, yet inversely correlated with many transcription factor binding sites. 

The high correlations among many of the genomic annotations gave precedent for the use of an elastic-net regularization algorithm in order to suitably reduce the feature space while retaining possibly correlated annotations that could be predictive of TAD boundary formation. The regularization was performed after implementing each of the resampling techniques on all data resolutions within each cell line. Additionally, a unique predictor-type was included separately as the set of features. It was found that the elastic-net regularized the coefficients of far more [??? quantify] distance-type predictors to 0 compared to datasets with overlap count and percent predictor types, regardless of cell line or data resolution (Supplementary Figure 3). Many elastic-net models using overlap count or percent predictors were reduced to ridge, or near-ridge regression models at higher data resolutions by having the $alpha$ penalization term tuned to 0 in the cross-validation step, thereby not reducing, or reducing very few of the feature space. The results of the variable reduction step imply that far more information relating annotions to TAD boundaries is encoded in the distance-type predictors. That is, the elastic-net algorithms were better able to separate noise and reduce the feature space when performed on distance-type predictors. Likewise, this reduction was most efficient at more refined levels of data resolution.

## MODELS WITH DISTANCE-TYPE PREDICTORS OUTPERFORM MODELS WITH OTHER PREDICTOR-TYPES

After suitably reducing the feature space, we performed individual random forest classification models specific to cell line, data resolution, resampling technique, and predictor type. We assessed each models' ability to correctly identify the genomic bins that overlap with flanked TAD boundaries using F1-Score, MCC, and balanced accuracy. For 10 kb data resoltion, it was found that models with distance-type predictors performed better according to all metrics, across each cell line, regardless of resampling technique used (Figure 4). This result was consistent across each of the other data resolutions as well (Supplementary Figure 4). Notably, we saw marked improvements in performance for models implemented on resampled datasets when compared to no resampling. 
...
We then directly compared models using SMOTE resampled datasets across each data resolution to see how performances changed as resolution increased. For each cell line, we found that model performances tended to decline as data resolution increased. Random forest models implemented on 10 kb, SMOTE resampled, data resolutions, with distance-type predictors were found to perform best according to MCC and balanced accuracy (Figure 5). There was a slight improvement in F1-Score for the 25 kb resolution on the GM12878 cell line. Performances from the K562 cell line degraded much faster as resolution increased, while the worst performing models for the GM12878 cell line were at the 50 kb resolution, followed by a slight improvement in performance at the 100 kb resolution. To be comprehensive we evaluated the performances across resolutions of models with distance-type predictors for the other resampling techniques and found the results to be largely consistent (Supplementary Figure 5). 
