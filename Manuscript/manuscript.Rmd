...
Publicly available topologically associating domain data was obtained from GEO with accession GSE53525 for the GM12878 cell line (Rao et. al).  The domain data was constructed from in situ Hi-C contact matrices at a 5kb resolution. Identified TAD boundaries were represented by their location in the genome (hg19 human genome assembly), including chromosome, start and end coordinates for chromosomes 1 through 22. The start and end coordinates were concatenated, sorted, and unique coordinates representing the borders demarcating TADs were obtained. 
...
When binning the genome, the sparcity of TAD boundaries contributed to heavily imbalanced classes. That is, proportionally, only a small number of genomic bins overlapped with a flanked TAD boundary. To assess this issue a series of multiple logistic regression models were performed. First, a multiple logistic regression model using the full data with unbalanced classes was assessed. We then compared this to a multiple logistic regression model with balanced classes created by random over-sampling, random under-sampling, and four other multiple logistic regression models with balanced classes created by the SMOTE (Synthetic Minority Over-sampling Technique) function in the DmWR package in R. For each of the random over- and under-sampling methods, multiple iterations were considered up until the difference in the cumulative means reached below a threshold of $5 \times 10^{-5}$. Performances were then aggregated by taking the average across all iterations. It was determined that the model with data untilizing random under-sampling out-performed all other other balancing techniques. However, to this point we only considered overlaps between genomic bins and the regions that defined functional genomic elements. 

### Incorporating Distances

To gain further insight into the spatial relationship between these chromatin factors and the genome, we introduced distance predictors into the model. This consisted of the distance from the center of each genomic bin to the center of the nearest region defining a genomic element as depicted in Figure X. To assess any improvement with the inclusion of these predictors we compared the performances of three multiple logistic regression models: one with only the overlap predictors, one with only the aforementioned distance predictors, and one with both.  It was found that the inclusion of distance predictors improved model performance.  Likewise, the model with both types of predictors futher improved model performance. As is common, with genomic data, the use of the distance predictors resulted in highly right skewed data with distances ranging from 0 to tens of thousands in base pairs. 

To better improve computational efficiency with regards to the convergence of likelihood maximization, we considered three normalization techniques. The first was a simple $log_{2}$ transformation, in which one was added in the case of distances of 0. The second was an inverse hyperbolic sine tranformation given by $insert function here$ which has the benefit of handling values of 0. Lastly, an inverse normal tranformation was considered. The inverse normal transformation is a way of transforming the sample distribution of a continuous variable to make it appear more normally distributed by matching the normalized rank of the trait to a quantile in a normal distribution using the following formula $insert formula here$. Multiple logistic regression models were performed using data with each respective transformation and compared. It was found that the model with inverse normally transformed distance predictors not only out-performed the other transformation but also out-performed a model with no transformation at all. 

### Classification Models

Therefore, the final data used for quantifying the association between each functional genomic element and TAD boundary formation was one that had been balanced using multiple iterations of random under-sampling, incorporated both overlapped and distance predictors, and normalized using an inverse normal transformation. Likewise, prior to model fitting, the predictors were standardized in order to compare magnitudes of the subsequent beta coefficients as a measure of variable importance.  We then performed a wide variety of classification algorithms to determine which genomic elements were most predictive of TAD boundaries. The models included a multiple logistic regression, a multiple logistic regression with both LASSO and Elastic-Net regularization, a random forest, a gradient boosting machine, a support vector machine, and a naiive bayes model. For each model, 10-fold cross validation was performed to tune for any neccessary paramenters. Model performance was compared using F1-scores, MCCs, and AUROCs. The alpha level of significance was set at 0.05. All statistical analyses was performed in R version 3.4.2.

# RESULTS

A total of 9048 topologically associating domains and their genomic locations were obtained for the GM12878 cell line for chromosomes 1 through 22.  The median width of TADs was found to be 18.5 kb (IQR: 11.5kb - 33.0kb). The start and end coordiantes of the TADs were translated into a vector of boundary points. There were 1454 (16%) TADs that shared at least one coordinate (start, end, or both) with another TAD. In this case the unique boundary point demarcating two adjacent TADs was considered. The genome was binned at 10 kb interval, resulting in a total of 536396 bins. A total of 536396 genomic bins of 10 kb width were created. After flanking the TAD boundary points on either side by 5 kb, it was found that 31820 (5.9%) bins contained a flanked TAD boundary. We investigated a total of 68 genomic features, 12 (18%) of which were removed because they were assessed as having near-zero variance. Table X presents a full list of the genomic features in addition to the metrics and indicator corresponding to whether or not the feature was to be removed. 

## Balancing Classes Improves Performance

It was found that the multiple logistic regression models utilizing random over and under-sampling out-performed the other techniques and were comparatively similar with F1 scores of 0.636 and 0.637 respectively. The under- sampling technique was evaluated over 13 iterations, while the over-sampling technique took far more iterations for the cumulative mean F1 score to converge, 26 to be exact. Given that and the fact that the under-sampling technique uses significantly less data than the over-sampling, while still performing at the same level, it was concluded that this was the best balancing technique and used in further analyses.

## Incorporating Distance Predictors Improves Performance

The multiple logistic regression model with only distance predictors, evaluated on multiple iterations of random under-sampling was found to improve model performance from 0.637 (using only overlap predictors) to 0.747. Furthermore, the multiple logistic regression model with both types of predictors further improved model performance by raising the F1 score to 0.753. This suggested that the distance predictors were more representative of the association between the genomic elements and TAD boundaries. However, on a genomic scale, distances from the bins to the nearest region defined by a respective genomic element were highly right skewed. We evaluated three tranformations in order to normalize the distance predictors and improve model performance. Out of the three normalization techniques, the multiple logistic regression model with an inverse normal transformation on the distance predictors was found to perform the best with an F1 score of 0.747. 

## X Classification Algorithm Provides Best Performance
