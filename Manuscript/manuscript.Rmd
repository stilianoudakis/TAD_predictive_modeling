...
## CLASS IMBALANCE SEVERELY IMPACTS PERFORMANCE OF MODELS WITH OVERLAP COUNTS PREDICTOR TYPES 

We first evaluated model performances using only the overlap counts predictor types without re-sampling as these characteristics represented models with the least granularity and could act as a baseline. We measured the performances across resolutions, for each cell line, and found that . 

Some notable results became clear. First, we see that the conventional AUC value was unable to accurately portray the poor performance of the models when compared to the composite metrics F1-score, MCC, and, to a lesser extent, the balanced accuracy. Inspecting the F1-score and MCC more closely, we see that some models at lower resolutions (for either cell line) were unable to correctly predict any bins that overlap with flanked TAD boundaries in the test data, as indicated by the missing bars.
