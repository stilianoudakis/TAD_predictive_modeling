...
Lastly, we evaluated models when using distance type predictors for the feature space. For the 5kb resolution data, tt was found that models with distance type predictors out performed models with count overlaps types across resolutions (Figure ). 

This result was also observed when compared to models with percent overlap type predictors, regardless of the variable reduction technique used (Supplementary Table). The differences in performances was found to be most severe when looking at the F1-score and MCC (Supplementary Table). 
