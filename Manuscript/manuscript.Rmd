...
We first compared models with either overlap counts or overlap percent predictor types, from the 5kb resolution data (Figure 12). These results were from random forest models fit to data without any re-sampling, using either LASSO or Elastic-Net regulariations. We found that differences in performances across variable reduction technique were marginal. However, when evaluating model performance using accuracy or AUC, the model appeared to perform relatively well. At second glance, though, when considering both the F1-score and MCC of the model, we were able to assess just how poorly the model was indeed performing (Supplementary Table 9). The low, and sometimes missing, values indicate that the model is unable to identify many, if any, true positive outcomes. That is, given an imbalanced data, the model is unable to predict which genomic bins truly contain a flanked TAD boundary. The performance results were found to be consistently poor for models with no re-sampling across the other resolutions (Supplementary Figure 13).

###Random UNDER-SAMPLING IMPROVES MODEL PERFORMANCE IN ALL RESOLUTIONS
