...
In this study we have developed an ensemble framework for predicting TAD boundaries that addresses class imbalance problem. We evaluated Hi-C data for the GM12878 cell line obtained at 10 kb, 25 kb, 50 kb, and 100 kb resolution. For each resolution, we examined the inclusion of three different predictor types, including distance as the measure of association between genomic elements. We then tested four re-sampling techniques aimed at creating balanced classes. Random forest classification was performed to both compare each combination of the above ensemble, and investigate which functional genomic features were most associated with the TAD boundaries.  Model performance was evaluated using accuracy, area under the precision-recall curve (AUPRC), F1 score, and Matthew's Correlation Coefficient (MCC). This process was then repeated for X additional cell lines including K562, ......

# METHODS

## DOMAIN DATA

Replicate Hi-C data for the GM12878, K562, ... cell lines were downloaded from GEO GSE63525 (Experiments HIC001 to HIC018, HIC069 to HIC 074, ... respectively; produced using the MbolI restriction enzyme) [@rao20143d]. [??? Need supplementary ref.] The autosomal genomic coordinates (GRCh37/hg19 human genome assembly) of TAD boundaries were obtained from data at 10 kb, 25 kb, 50 kb, and 100 kb resolution using the Arrowhead algorithm [@durand2016juicebox]. The start and end coordinates of all TADs were concatenated, sorted, and unique TAD boundary points were obtained. 
...
3. **Log2 Distance**: For each genomic bin, the $log_{2}$ of the distance (refered to hereafter simply as $\textit{distance}$), in basepairs, from the center of the bin to the center of the nearest region of a respective genomic annotation region [??? Decide whether to use "annotation" or "feature"] was calculated. The log transformation was used as a normalization technique in order to account for the skewness of the data (Supplementary Figure 1).
...
The efficacy of different ensemble systems was compared using various performance metrics including accuracy, F1-score, and MCC. These metrics are defined as follows:
...
For the task of identifying the functional genomic elements that were most predictive of the formation of TAD boundaries, we evaluated the variable importances associated with each predictor of the random forest models associated with each resampling technique. For each model and within each tree, the prediction accuracy on the out-of-bag portion of the data was recorded. Then the same was done after permuting each predictor variable. The difference between the two accuracies was then averaged over all trees, and normalized by the standard error. All random forest algorithms were implemented in R using the `caret` package version 6.0 [@kuhn2012caret]. 

# RESULTS

## ENSEMBLE COMPARISONS

We evaluated and compared multilpe random forest classification algorithms built on domain data extracted from Hi-C datasets for X cell lines C = {GM12878, K562, ...}, at four different resolutions R = {10 kb, 25 kb, 50 kb, 100 kb}. For each classifier, we compared 3 predictor types P = {OC, OP, Distance} and 5 class balancing techniques B = {None, RUS, ROS, SMOTE, ROSE}. A system produced from the ensemble framework was defined as: E = {c, r, p, b}, where $c \in C, \quad R \in R, \quad p \in P,$ and $b \in$ B. Letting |X| denote the cardinality of each set, we evaluated $|C| \times |R| \times |P| \times |B|$ different ensemble systems, totaling X (=$x \times 4 \times 3 \times 5$) separate models. For simplicity, we present the most significant results, with the complete results available as Supplementary Tables and Figures.

## DISTANCE TYPE PREDICTORS OUTPERFORM OTHER PREDICTOR-TYPES

Prior to resampling, we performed a series of random forest classification algorithms on the full features space (all combined predictor types), as well as on type-specific feature spaces (overlap counts, overlap predictors, and log2 distances) to evaluate which predictor types were best able to discriminate between genomic bins that either flanked or did not flank TAD boundaries. It was found that, for 10 kb resolution data, models with distance type predictors had higher AUPRC values compared to other predictor types for both GM12878 and K562 cell lines (Figure 4). This result was observed across all other data resolutions as well (Supplementary Figure 2). There was a gradual decline in AUPRC values as resolution increased with models built on 100 kb resolution domain data having the lowest AUPRC curves, regardless of predictor type (Supplementary Table 2).

Recursive feature elimination was used to establish the optimal number of distance-type features to include in downstream analyses. It was determined that, for 10 kb resolution data, near optimal performance was obtained for models with only 32 predictors [??? Distance-based only?] for each cell line (Figure 5). Results remained consistent across the other data resolutions (Supplementary Figure 3). Therefore, we reduced the feature space for subsequent models to the top 32 predictors, respective to resolution and cell line. The sets of top predictors chosen varied across resolution and cell line due to differences in the parameters provided in the TAD calling algorithm (Supplementary Table 3). For the 10 kb resolution data, the most important predictive features that were robust across cell lines were a mixture of transcription factor binding sites, chromatin states, and histone modifications including Smc3, Rad21, CTCF, and H3k9 trimethylation (Figure 6). [??? We don't care - these are for unbalanced dataset] Transcription factor binding sites Smc3, Rad21, and CTCF were also consitently featured as top predictors when comparing cell lines across the the 25 kb and 50 kb resolutions (Supplementary Figure 4). Results when cell lines were compared across the 100 kb resolutions were less consitent with current literature.

## DATA RESAMPLING IMPROVES MODEL PREDICTION

The low baseline thresholds represented by the dashed line in the precision-recall curves (Figure 4 and Supplementary Figure 2) indicate the wide disparity between majority and minority classes. Ideally a manageable dataset will have an imbalance ratio hovering near 1. However, in our study we found that there was an average imbalance ratio of 0.154 (SD=0.022) and 0.137 (SD=0.0181) for cell lines GM12878 and K562 respectively (Supplementary Figure 5).  This highlights the need for suitable resampling techniques. 
