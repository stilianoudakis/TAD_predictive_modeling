...
The distinct patterns of chromatin states, active or repressive histone marks, binding peaks of different factors associated with the 3D structures [@Naumova:2010aa; @Hou:2012aa] prompt the use of computational approaches to identify which epigenomic features are most predictive of TAD boundaries. Current methods rely on the use of classification algorithms to draw associations between the spatial relationship of functional genomic elements and regions that include TAD boundaries. Although several useful insights have been made, such as the convergent binding of CTCF [@ong2014ctcf; @ghirlando2016ctcf], [??? Describe other hallmarks of TAD boundaries, with references. And, most importantly, why you are doing your study?]
...
To establish the data for modeling, the genome was binned into equally sized intervals defined by the data resolution for the corresponding contact matrix (Figure 1). For example, for domain data obtained from a 10 kb resolution contact matrix, the genome was binned into 10 kb intervals. For the boundary data, in order to accommodate for the uncertainty in exact boundary location, the boundary points were flanked on both sides by the size of the resolution of the corresponding contact matrix. For example, for 10 kb resolution data, each uniquely identified TAD boundary was flanked on either side by 10 kilobases for a total width around each boundary point of 20 kb. A genomic bin was then labeled as having a TAD boundary within it (Y=1) if it overlapped with a particular flanked boundary. Otherwise, the bin was labeled as not containing a boundary (Y=0). The process was repeat for each subsequent resolution.
...
The efficacy of each ensemble system in the framework was compared using composite metrics including F1-score and MCC. These metrics are defined as follows:
...
Multiple random forest classification algorithms [??? What you mean by multiple? Random forest already has multiplicity in it.] were built on domain data extracted from Hi-C data for 2 cell lines $C = \{\textit{GM12878}, \textit{K562}\}$, at four different resolutions $R = \{\textit{10 kb}, \textit{25 kb}, \textit{50 kb}, \textit{100 kb\}}$. [??? Bad spacing, for the whole paragraph. Why you are not using plain and simple Markdown?] For each classifier, we compared model performances using 3 separate predictor types $P = \{\textit{OC}, \textit{OP}, \textit{Distance}\}$ and 4 class balancing techniques $B = \{\textit{None}, \textit{RUS}, \textit{ROS}, \textit{SMOTE}\}$. A system produced from the ensemble framework was defined as: $S = \{\textit{c}, \textit{r}, \textit{p}, \textit{b}\}$, where $c \in C$, $r \in R$, $p \in P$, and $b \in B$. Letting $|X|$ denote the cardinality of each set, we evaluated $|C| \times |R| \times |P| \times |B|$ different ensemble systems, totaling $96 (= 2 \times 4 \times 3 \times 4)$ separate models. 

## IMBALANCE RATIOS REMAINED SEVERE AND CONSISTENT ACROSS RESOLUTIONS

To assess the full [???] severity of class imbalance present when constructing the models, we investigated the relationship between the total number of bins (sample observations) included at each resolution [??? That's mundane, supplemental at best] and the subsequent imbalance ratio (IR). [??? Did you ever defined it?] As expected, the total number of genomic bins decreased as resolution decreased (Figure 4A). Specifically, at lower resolutions, the linear genome was binned into larger units, thereby creating fewer bins. however, the IRs remained relatively stable and consistent across resolutions and cell lines (Figure 4B). We found that there was an average IR of 0.154 (SD=0.022) and 0.137 (SD=0.0181) across resolutions for the GM12878 and K562 cell lines respectively. Here, the low imbalance ratios indicated a wide disparity between the number of genomic bins that overlap with a flanked TAD boundary and the much larger number that did not. 
...
In order to set a baseline in performance, we first evaluated the least granular model, that is one implemented using OC predictor types, without data re-sampling. In evaluating such composite metrics like F1-Score and MCC, we found that for the GM12878 cell line, these models exhibited poor performances across all resolutions (Figure 5A). <!--The average F1-Score and MCC across resolution was found to be 0.04 and 0.07 respectively.--> 

We then gradually increased the granularity in the characteristics of the model by assess the model built on OP predictor types, while still not performing any data resampling. Here we found that performances were worse than when using OC predictor types for the GM12878 cell line (Figure 5B). <!--Here, the average F1-Score and MCC was found to be 0.01 and 0.03 respectively.--> Finally, models built on distance-type predictors in conjuction with no resampling, however, were found to exhibit slightly better performances than the other two types<!--, with average F1-Scores and MCCs of 0.08 and 0.10 respectively--> (Figure 5C). 

Importantly, when validating these 3 models built on fully imbalanced datasets, the F1-Score and MCC could not be determined at lower resolutions given by the model's complete inability to correctly classify any genomic bins that overlapped with flanked TAD boundaries in the test data, thereby resulting in 0 true positives. Furthermore, we found that evaluating models using conventional metrics, such as AUC or accuracy, did not depict the true severity in performance (Supplementary Table 3). <!--The average AUC across resolutions for the OC, OP, and distance type predictors was 0.65, 0.66, and 0.85 respectively, where as the average accuracies were 0.89, 0.89, and 0.90 respectively (Supplementary Table 3).--> Results were found to be consistently poor across resolutions for the K562 cell line (Supplementary Figure 10). Similarly, many of the performance metrics at higher resolutions were also unable to be reported due to the models inability to produce any true positive cases.

## DATA RE-SAMPLING COUPLED WITH THE INCLUSION OF A DISTANCE-TYPE FEATURE SPACE IMPROVED PERFORMANCES AMONG ALL CLASSES OF MODELS 

Comparisons of the effects of the various resampling techniques were then made. When models were built on randomly oversampled (ROS) data, we found a substantial increase in F1-Score and MCC for the GM12878 cell line (Figure 5). The biggest improvement in performance was seen at the 10 kb resolution, with a downward trend as resolutions decreased. Although performances were low at lower resolutions, both F1-Scores and MCC values were able to be obtained, contrary to models built on completely unbalanced data as seen before. Distance-type predictors were found to offer the highest F1-Scores and MCCs for ROS data, across all resolutions. Similar results were obtained when applying ROS to the K562 cell line (Supplementary Figure 10). Here, a much sharper negative trend was featured for both F1-Score and MCC and resolution decreased, than in the GM12878 cell line.

Further improvement in performance was found when models were implemented on both randomly undersampled (RUS) and SMOTE resampled data, for all 3 predictor types (Figure 5). Although the differences in performance between the two techniques were marginal, SMOTE resampled data was found to yield the best F1-Scores and MCCs among all classes of models developed and compared in the ensemble framework. As with ROS, distance type predictors yeilded the highest F1-Score and MCC values across resolutions, when applied to either RUS or SMOTE. SMOTE re-sampled data, with the inclusion of distance-type predictors was also found to be the best set of characteristics when applied to the K562 cell line (Supplementary Figure 10). 

Moreover, we found that there was a degredation in model performance as data resolution decreased. This was much more pronounced in the K562 cell line. On the GM12878 cell line, there were only marginal differences in performance between 10 kb and 25 kb, followed by a steep decline at 50 kb. Then, both F1-Scores and MCCs showed a momentary increase in performance at 100 kb, the lowest resolution considered in the study. Performances from the K562 cell line degraded much faster as resolution decreased, starting with highest performance at 10 kb and ending with the worst performance at 100 kb.

These results imply that a best performing model, one that is able to best discriminate between genomic bins that are near versus far away from TAD boundaries, is one that incorporates both random under- and over-sampling, as SMOTE is known to do.  Additionally, when we performed and evaluated the variable importances of random forest models built on one genomic annotation at a time (including the 3 predictor types each), we found that the distance-type consistently out-ranked the OC and OP type predictors in terms of predictive importance, for a majority of the cell line specific annotations, across resolution and cell line (Supplementary Tables 4 & 5). Therefore, it appears that distance-type predictors provided the most informative spatial representation of epigenomic annotations associated with TAD boundaries.

## VARIABLE IMPORTANCES FROM TOP PERFORMING MODELS CONFIRMED BIOLOGICAL FUNCTIONALITY OF KNOWN TRANSCRIPTION FACTOR BINDING SITES

In addition to adressing both the affect of class imbalance and the optimal spatial relationships used when predicting TAD boundaries, we also aimed at identifying the genomic annotations that were most predictive of TAD boundary formation. In doing so, we investigated which genomic annotations yielded the hight predictive importance values for random forest models implemented on the best given set of data-level characteristics determined above (SMOTE re-sampled with distance type predictors), across resolution and cell line. We found that specific transcription factor binding sites (TFBS), including SMC3ab, ZNF143, CTCF, and RAD21, were given consistently high predictive importance in the models built on the GM12878 cell line, across all resolutions (Figure 8). These are known factors associated with the hypothesized loop extrusion model [@fudenberg2016formation; @alipour2012self] and were found to yield consistently high predictive importance on the K562 cell line as well, with similar profiles across resolutions (Supplementary Figure 15). Other annotations known to be associated with gene regulation and affect structural properties of chromatin, such as the YY1  transcription factor, as well DNAse hypersensitive sites, regions of heterochromatin, and the H3k27 trimethylation histone modifications were ranked high on the 10 kb resolution, but were less consistently ranked at lower resolutions for both cell lines. <!--Moreover, the ranking profiles of genomic annotations for both cell lines were found to be more similar between the 10 kb and 25 kb resolutions, as well as between the 50 kb and 100 kb resoluions (this is proved when looking at a clustered heatmap).-->

## MODELS IMPLEMENTED ON TRANSCRIPTION FACTOR BINDING SITES OUTPERFORM OTHER ANNOTATION-SPECIFIC MODELS

Given the prevalence of TFBS as the most consistently top ranked predictive features across resolution and cell line, we ventured to further compare performances of annotation specific models. After applying SMOTE re-sampling with distance type predictors, random forest models built on sets of histone modifications, chromatin state segmentations, and TFBS were compared to a model that included all three types together. For the GM12878 cell line, models built simply on TFBS were found to perform just as well as models that included all 3 sets of annotations according to F1-Score and MCC, consistently across resolution (Figure 6). Similar results were found when evaluating the K562 cell line (supplementary Figure 11). For both resolutions, models built on only histone modifications were found to have the lowest F1-Scores and MCC, across resolutions, followed by models built on chromatin state segmentations. 
