...
Distinct patterns in epigenomic marks, binding peaks of different factors, and actively transcribed genes prompt the use of computational approaches to identify which epigenomic features are most predictive of TAD boundaries [@dixon2015chromatin; @di2017novo] [??? REFS? Carefully select from my paper notes; !!!added refs]. Recent research has identified key functional genomic annotations enriched at boundaries. Namely, the zinc finger protein, CTCF, has been shown to be associated with TAD boundaries [@ghirlando2016ctcf]. Other transcription factors including SMC1, SMC3, and RAD21 have been shown to colocalize near TAD boundaries [@sanborn2015chromatin; @fudenberg2016formation; @alipour2012self]. Together, these components are hypothesized to be what causes loops in the DNA, contributing to higher order chromatin organization. Additional binding proteins, known to mediate enhancer-promoter interactions including YY1 and ZNF143, have been shown to influence the delineation of topological domains [@weintraub2017yy1; @gonzalez2017znf143; @heidari2014genome]. <!--However, there is a growing number of publically available annotated genomic features, and it is unclear if current methods are robust enough to identify all of them. [??? This is very weak and undefined. You started with mentioning prediction - finish with unsolved problems with it. !!!editted]-->

[!!! This paragraph defines problems]

Current methods used to predict TAD boundaries rely on the use of classification models to draw associations between functional annotations and regions that include TAD boundaries. However, <!--this is a multi-faceted probelem. [??? "WATER"!]-->many key characteristics of 3D genomic data remain unexplored, potentially limiting performance of predictive modeling. <!--including sparsity of TAD boundaries as well as suitably describing the spatial relationship between annotations and TAD boundaries.-->Specifically, the minority of TAD boundaries as compared with the rest of the genome creates heavily imbalanced classes, characterized by ratios of positive/negative classes used to train the models. The imbalance ratio is defined as the ratio of minority classes (regions with TAD boundaries) to majority classes (regions without TAD boundaries) [@Orriols-Puig2008]. [??? You confused me a lot with this] Such heavily imbalanced datasets <!--are known to [??? WATER!] -->adversely impact the performance of the classifiers as the learned model is biased towards the majority class [@lusa2010class; @chen2013influence; @jeni2013facing]. Furthermore, the use of different levels of association between TAD boundaries and genomic features (number, or proportion of overlaps, or distance) as predictors remain underexplored. Lastly, it remains unclear how the increased resolution of Hi-C data affect the performance of predictive modeling. 

[!!! The following paragraph should review previous work and highlight pitfalls, creating motivation for yor study]

Recently, an L1-regularized multiple logistic regression model was developed to predict TAD boundaries using transcription factor binding sites on GM12878 [@mourad2016computational]. Based on a 1 kb binned genomic data matrix and 20 kb flanked TAD boundaries, the model was trained to identify the set of transcription factors most influential of TAD boundaries. Several problems exist with the study. First, the class imbalance issue was not addressed. Secondly, many transcription factors are known to colocalize, and therefore exhibit high correlations. If a group of predictors are highly correlated among themselves, L1-regularization tends to pick only one of them and will shrink the others to zero [@zou2005regularization].  Thirdly, [@mourad2016computational] reported ROC (receiver operating characteristic) curves and subsequent AUC (area under the curve) as measures of their model's performance. It has been shown that AUC is insensitive to class imbalance, creating artificially inflated values inlfuenced by the majority class [@prati2011survey; @wei2013role].
...
To investigate class imbalance, defined as the ratio of TAD vs. non-TAD regions, we estimated the number of TAD boundaries ($Y=1$ in the response vector) and the rest of genomic bins ($Y=0$) at each of the four resolutions of Hi-C data. Expectedly, the average number of TAD boundaries and the rest of genomic bins increased with the increased resolution of Hi-C data (Table 1) irrespectively of cell line (Supplementary Tables 3). However, imbalance ratios were found to be invariant to resolution, remaining consistently low with very little variability between chromosomes, regardless of cell line.
