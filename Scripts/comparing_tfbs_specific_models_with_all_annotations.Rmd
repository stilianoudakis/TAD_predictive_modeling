...
#GM12878
ggplot(all_rfperfs_mcc[which(all_rfperfs_mcc$CL=="GM12878"),], aes(x=Resolution, y=MCC, group=Annotation, fill=Annotation)) +
...
#K562
ggplot(all_rfperfs_mcc[which(all_rfperfs_mcc$CL=="K562"),], aes(x=Resolution, y=MCC, group=Annotation, fill=Annotation)) +
  geom_bar(stat="identity", position = position_dodge()) +
  geom_errorbar(aes(ymin=MCC-MCCSD, ymax=MCC+MCCSD), width=.5,
                 position=position_dodge(.9), alpha=0.75) +
  theme_minimal() +
