...
for(i in c("GM12878","K562")){
  for(j in c("5kb", "10kb", "25kb","50kb","100kb")){
    for(k in 1:length(chrs)){
      ...
      pi.per.chr <- rbind(pi.per.chr, table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)[2]/sum(table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)))
