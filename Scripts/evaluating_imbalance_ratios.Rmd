...
for(i in c("GM12878","K562")){
  for(j in c("10kb", "25kb","50kb","100kb")){
    for(k in 1:length(chrs)){
      ...
      ir.per.chr <- rbind(ir.per.chr, table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)[2]/table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)[1])
...
irtab <- cbind.data.frame(cl.per.chr,
                          ...
                          bounds.per.chr,
                          ir.per.chr)
colnames(irtab) <- c("cl","res","chr","bins","bounds","ir")

irtab_summ <- irtab %>%
  group_by(cl, res) %>%
  summarise(AveBins = mean(bins, na.rm = TRUE),
