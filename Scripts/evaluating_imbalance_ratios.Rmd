...
for(i in c("GM12878","K562")){
  for(j in c("5kb", "10kb", "25kb","50kb","100kb")){
    for(k in 1:length(chrs)){
      ...
      ir.per.chr <- rbind(ir.per.chr, table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)[2]/(table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)[1]+table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)[2]))
      
      tads.per.chr <- rbind(tads.per.chr, dim(tad_dat[which(tad_dat$V1==k),])[1])
      
      pi.per.chr <- rbind(pi.per.chr, 2*(table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y)[2]/sum(table(mcols(binslist_dat_center[seqnames(binslist_dat_center)==chrs[k]])$y))))
...
irtab <- cbind.data.frame(cl.per.chr,
                          ...
                          ir.per.chr,
                          pi.per.chr)
colnames(irtab) <- c("cl","res","chr","bins","tads","bounds","ir","pi")
irtab$res <- factor(irtab$res, levels=c("5kb","10kb","25kb","50kb","100kb"))
...
irtab_summ <- irtab %>%
  ...
  dplyr::summarise(AveBins = mean(bins, na.rm = TRUE),
            ...
            AvePI = mean(pi, na.rm = TRUE),
            AvePISD = sd(pi, na.rm = TRUE))
...
irtab_summ_med <- irtab %>%
  ...
  dplyr::summarise(MedBins = median(bins, na.rm = TRUE),
            ...
            MedPI = median(pi, na.rm = TRUE),
            MedPIIQR = IQR(pi, na.rm = TRUE))
...
##Boundary regions Plots

```{r}
a <- ggplot(irtab_summ[which(irtab_summ$res=="5kb"),], aes(x=cl, y=AveBounds, fill=cl))+
  geom_bar(stat="identity", position="dodge")+
  geom_errorbar(aes(ymin=AveBounds-AveBoundsSD, ymax=AveBounds+AveBoundsSD), width=.5,
                 position=position_dodge(.9), alpha=0.75) +
  theme_minimal() +
  theme_bw()+
  xlab("") + ylab("") + ylim(0,2100) +
  scale_fill_discrete(name="Cell Line") +
  scale_y_continuous(limits = c(0,2100),
                     breaks=c(0,500,1000,1500,2000),
                     labels=c("0"="0", "500"="5", "1000"="10", "1500"="15", "2000"="20")) +
  ggtitle("5 kb") +
  guides(fill=FALSE) +
  theme(axis.text.x = element_text(size=15,
                                   angle = 45, 
...
b <- ggplot(irtab_summ[which(irtab_summ$res=="10kb"),], aes(x=cl, y=AveBounds, fill=cl))+
  geom_bar(stat="identity", position="dodge")+
  geom_errorbar(aes(ymin=AveBounds-AveBoundsSD, ymax=AveBounds+AveBoundsSD), width=.5,
                 position=position_dodge(.9), alpha=0.75) +
  theme_minimal() +
...
c <- ggplot(irtab_summ[which(irtab_summ$res=="25kb"),], aes(x=cl, y=AveBounds, fill=cl))+
  geom_bar(stat="identity", position="dodge")+
  geom_errorbar(aes(ymin=AveBounds-AveBoundsSD, ymax=AveBounds+AveBoundsSD), width=.5,
                 position=position_dodge(.9), alpha=0.75) +
  theme_minimal() +
...
d <- ggplot(irtab_summ[which(irtab_summ$res=="50kb"),], aes(x=cl, y=AveBounds, fill=cl))+
  geom_bar(stat="identity", position="dodge")+
  geom_errorbar(aes(ymin=AveBounds-AveBoundsSD, ymax=AveBounds+AveBoundsSD), width=.5,
                 position=position_dodge(.9), alpha=0.75) +
  theme_minimal() +
...
e <- ggplot(irtab_summ[which(irtab_summ$res=="100kb"),], aes(x=cl, y=AveBounds, fill=cl))+
  geom_bar(stat="identity", position="dodge")+
  geom_errorbar(aes(ymin=AveBounds-AveBoundsSD, ymax=AveBounds+AveBoundsSD), width=.5,
                 position=position_dodge(.9), alpha=0.75) +
  theme_minimal() +
...
annotate_figure(figure,
                ...
                left = text_grob("Average number of TBR across chromosomes\n(thousands)", color = "black", rot = 90, size=20)
)
...
```{r}
a <- ggplot(irtab[which(irtab$res=="5kb"),], aes(x=cl, y = pi, fill=cl, color=cl))  + 
 stat_boxplot(geom ='errorbar', width = 0.2) + 
 geom_boxplot() +
 theme_minimal() +
 theme_bw()+
...
b <- ggplot(irtab[which(irtab$res=="10kb"),], aes(x=cl, y = pi, fill=cl, color=cl))  + 
 stat_boxplot(geom ='errorbar', width = 0.2) + 
 geom_boxplot() +
 theme_minimal() +
 theme_bw()+
...
c <- ggplot(irtab[which(irtab$res=="25kb"),], aes(x=cl, y = pi, fill=cl, color=cl))  + 
 stat_boxplot(geom ='errorbar', width = 0.2) + 
 geom_boxplot() +
 theme_minimal() +
 theme_bw()+
...
d <- ggplot(irtab[which(irtab$res=="50kb"),], aes(x=cl, y = pi, fill=cl, color=cl))  + 
 stat_boxplot(geom ='errorbar', width = 0.2) + 
 geom_boxplot() +
 theme_minimal() +
 theme_bw()+
...
e <- ggplot(irtab[which(irtab$res=="100kb"),], aes(x=cl, y = pi, fill=cl, color=cl))  + 
 stat_boxplot(geom ='errorbar', width = 0.2) + 
 geom_boxplot() +
 theme_minimal() +
 theme_bw()+
...
annotate_figure(figure,
    #bottom = text_grob("Cell Line", color = "black", size = 20),
    left = text_grob("Proportion of Imbalance", color = "black", rot = 90, size=20)
)
