...
```{r}
pcadat_gm12878_10kb_count <- pcadat_gm12878_10kb_count[which(rownames(pcadat_gm12878_10kb_count) %in% chromstatevars),]

##with loadings
autoplot(prcomp(pcadat_gm12878_10kb_count),
         loadings = TRUE,
         label = TRUE, 
         label.size = 3,
         label.repel=T) +
...
wss <- (nrow(pcadat_gm12878_10kb_count)-1)*sum(apply(pcadat_gm12878_10kb_count,2,var))
  for (i in 2:5) wss[i] <- sum(kmeans(pcadat_gm12878_10kb_count,
                                       centers=i, nstart=25, iter.max = 1000)$withinss)
plot(1:5, wss, type="b", xlab="Number of Clusters",
     ylab="Within groups sum of squares")

set.seed(1)
autoplot(kmeans(pcadat_gm12878_10kb_count, 3, nstart=25, iter.max = 1000),
         data=pcadat_gm12878_10kb_count,
         frame = TRUE,
         frame.type = 'norm',
         label = TRUE, 
         label.size = 3, 
