...
###Saving clustering annotations plots
...
## Clustering annotations (unseparated)

```{r}
pcaplotfunction <- function(cl, resolution){
  
  if(cl=="GM12878"){
    countdir = paste("Z:/TAD_data_analysis/", cl, "/", resolution, "/Ensemble/PCA/pcadat_gm12878_", resolution, "_count.rds",
                    sep="")
    percdir = paste("Z:/TAD_data_analysis/", cl, "/", resolution, "/Ensemble/PCA/pcadat_gm12878_", resolution, "_perc.rds",
                    sep="")
    distdir = paste("Z:/TAD_data_analysis/", cl, "/", resolution, "/Ensemble/PCA/pcadat_gm12878_", resolution, "_dist.rds",
                    sep="")
  }else{
    countdir = paste("Z:/TAD_data_analysis/K562/", resolution, "/Ensemble/PCA/pcadat_K562_count.rds",
                    sep="")
    percdir = paste("Z:/TAD_data_analysis/K562/", resolution, "/Ensemble/PCA/pcadat_K562_perc.rds",
                    sep="")
    distdir = paste("Z:/TAD_data_analysis/K562/", resolution, "/Ensemble/PCA/pcadat_K562_dist.rds",
                    sep="")
    }
  ...
  data <- (countdata + percdata + distdata)/3
...
tadclusterplotfunction <- function(cl, resolution, predictortype, cluster){
  ...
  #if(length(nzvar) > 0){
  #  data <- data[, -which(colnames(data) %in% nzvar)]
  #}
  
  ##keeping top 75th percentile variable
  colvariance <- apply(data,2,var)
  thirdquart <- as.numeric(summary(colvariance)[5])
  data <- data[,which(colvariance >= thirdquart)]
