...
all_rfperfs$Resolution <- rep(c(rep("10 kb", 252),
                            rep("25 kb", 252),
                            rep("50 kb", 252),
                            rep("100 kb", 252)))
all_rfperfs$Resampling <- rep(c(rep("None", 3),
                            ...
                            rep("RUS", 3),
                            rep("SMOTE", 3)),168)
all_rfperfs$Predictor <- rep(c("Distance", "OC", "OP"), 672)
...
all_rfperfs_mcc <- all_rfperfs %>%
  group_by(CL, Resolution, Resampling, Predictor) %>%
  summarise(MCC = mean(V1, na.rm = TRUE),
