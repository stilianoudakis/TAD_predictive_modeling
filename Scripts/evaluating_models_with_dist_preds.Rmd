...
```{r}
figure <- ggarrange(p.none, p.ros, p.rus, 
          ncol=1, nrow=3, 
          common.legend = TRUE, 
          legend="top",
          labels="AUTO",
...
```{r}
all_perfs <- rbind.data.frame(all_None_perfs,
                              all_ROS_perfs,
                              all_RUS_perfs,
                              all_smote_perfs)
all_perfs$Resampling <- c(rep("None",24),
                          rep("ROS",24),
                          rep("RUS",24),
                          rep("SMOTE",24))
all_perfs$CL <- factor(all_perfs$CL, levels=c("GM12878","K562"))
all_perfs$Resolution <- factor(all_perfs$Resolution, levels=c("10 kb", "25 kb", "50 kb", "100 kb"))
all_perfs$Metric <- factor(all_perfs$Metric, levels=c("Balanced Acc", "F1-Score", "MCC"))
all_perfs$Resampling <- factor(all_perfs$Resampling, levels = c("None", "ROS", "RUS", "SMOTE"))

#gm12878
p1 <- ggplot(all_perfs[which(all_perfs$CL=="GM12878"),], aes(x=Resolution, y=Performance, color=Resampling, group=Resampling)) +
    geom_line(size=2)+
    geom_point(size=5)+
    facet_grid(~ Metric)+
    theme_minimal() +
...
#k562
p2 <- ggplot(all_perfs[which(all_perfs$CL=="K562"),], aes(x=Resolution, y=Performance, color=Resampling, group=Resampling)) +
    geom_line(size=2)+
    geom_point(size=5)+
    facet_grid(~ Metric)+
    theme_minimal() +
