...
called_and_pred_17 <- readRDS(paste0("Z:/TAD_data_analysis/GM12878/10kb/results_by_chr/CHR17/smote/distance/enet/called_and_pred_bound_regions.rds"))
true_bound_17 <- called_and_pred_17[[3]]
pred_bound_17 <- called_and_pred_17[[4]]
rm(called_and_pred_17)

called_and_pred_18 <- readRDS(paste0("Z:/TAD_data_analysis/GM12878/10kb/results_by_chr/CHR18/smote/distance/enet/called_and_pred_bound_regions.rds"))
true_bound_18 <- called_and_pred_18[[3]]
pred_bound_18 <- called_and_pred_18[[4]]
rm(called_and_pred_18)

called_and_pred_19 <- readRDS(paste0("Z:/TAD_data_analysis/GM12878/10kb/results_by_chr/CHR19/smote/distance/enet/called_and_pred_bound_regions.rds"))
true_bound_19 <- called_and_pred_19[[3]]
pred_bound_19 <- called_and_pred_19[[4]]
rm(called_and_pred_19)

called_and_pred_20 <- readRDS(paste0("Z:/TAD_data_analysis/GM12878/10kb/results_by_chr/CHR20/smote/distance/enet/called_and_pred_bound_regions.rds"))
true_bound_20 <- called_and_pred_20[[3]]
pred_bound_20 <- called_and_pred_20[[4]]
rm(called_and_pred_20)

called_and_pred_21 <- readRDS(paste0("Z:/TAD_data_analysis/GM12878/10kb/results_by_chr/CHR21/smote/distance/enet/called_and_pred_bound_regions.rds"))
true_bound_21 <- called_and_pred_21[[3]]
pred_bound_21 <- called_and_pred_21[[4]]
rm(called_and_pred_21)

called_and_pred_22 <- readRDS(paste0("Z:/TAD_data_analysis/GM12878/10kb/results_by_chr/CHR22/smote/distance/enet/called_and_pred_bound_regions.rds"))
true_bound_22 <- called_and_pred_22[[3]]
pred_bound_22 <- called_and_pred_22[[4]]
rm(called_and_pred_22)

all_true_bounds <- GRangesList(true_bound_17,
                               true_bound_18,
                               true_bound_19,
                               true_bound_20,
                               true_bound_21,
                               true_bound_22)
all_pred_bounds <- GRangesList(pred_bound_17,
                               pred_bound_18,
                               pred_bound_19,
                               pred_bound_20,
                               pred_bound_21,
...
all_true_bounds <- do.call("c", all_true_bounds)
all_pred_bounds <- do.call("c", all_pred_bounds)
...
EnrichedHeatmap(mat, 
                ...
                top_annotation = HeatmapAnnotation(lines = anno_enriched(gp = gpar(col = muted("red"), 
                                                                                   lwd = 2), 
                                                                         yaxis_side = "right", 
                                                                         yaxis_facing = "left")))
```
...
```{r}
pred_v_called_10_df <- data.frame(LogDist = c(#Predicted
    log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1),
    log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[2]]))$distance+1),
    log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[3]]))$distance+1),
    log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[4]]))$distance+1),
    
    #Called
    log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1),
    log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[2]]))$distance+1),
    log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[3]]))$distance+1),
    log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[4]]))$distance+1)
),
Annotation = c(#Predicted
    rep("CTCF",length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("RAD21",length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("SMC3a",length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("ZNF143",length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    
    #Called
    rep("CTCF",length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("RAD21",length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("SMC3a",length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("ZNF143",length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1)))),
BoundReg = c(#Predicted
    rep("Predicted", length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("Predicted", length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("Predicted", length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("Predicted", length(log2(mcols(distanceToNearest(all_pred_bounds, annots.GR.f[[1]]))$distance+1))),
    
    #Called
    rep("Called", length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("Called", length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("Called", length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1))),
    rep("Called", length(log2(mcols(distanceToNearest(all_true_bounds, annots.GR.f[[1]]))$distance+1)))))
...
ggplot(pred_v_called_10_df, aes(x=BoundReg, y = LogDist, fill=BoundReg))  + 
    facet_grid(~ Annotation) +    
    stat_boxplot(geom ='errorbar', width = 0.2) + 
    geom_boxplot(outlier.shape = NA) +
    geom_signif(test = "wilcox.test", 
