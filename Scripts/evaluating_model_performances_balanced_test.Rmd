---
title: "evaluating_model_performances_balanced_test"
...
```{r}
mccdata_gm12878_10kb_bt <- numeric()
...
for(i in paste0("CHR", c(1:8,10:22))){
	for(j in c("none","ros","rus","smote")){
		for(k in c("distance", "oc", "op")){
			...
			mccdata_gm12878_10kb_bt <- rbind(mccdata_gm12878_10kb_bt,
										  perfdata[15,2])
		}
	}
}
...
```{r}
all_rfperfs <- rbind.data.frame(mccdata_gm12878_10kb_bt,
                                mccdata_gm12878_10kb)

all_rfperfs$Testing <- c(rep("Balanced", length(mccdata_gm12878_10kb_bt)),
                                rep("Unbalanced", length(mccdata_gm12878_10kb)))
...
all_rfperfs_mcc <- all_rfperfs %>%
  dplyr::group_by(Testing, Resampling, Predictor) %>%
  dplyr::summarise(MCC = mean(V1, na.rm = TRUE),
            MCCSD = sd(V1, na.rm = TRUE))

ggplot(all_rfperfs_mcc, aes(x=Predictor, y=MCC, fill=Testing)) +
...
```{r}
F1data_gm12878_10kb_bt <- numeric()
...
for(i in paste0("CHR", c(1:8,10:22))){
	for(j in c("none","ros","rus","smote")){
		for(k in c("distance", "oc", "op")){
			...
			F1data_gm12878_10kb_bt <- rbind(F1data_gm12878_10kb_bt,
										  perfdata[16,2])
		}
	}
}
...
```{r}
all_rfperfs <- rbind.data.frame(F1data_gm12878_10kb_bt,
                                F1data_gm12878_10kb)

all_rfperfs$Testing <- c(rep("Balanced", length(F1data_gm12878_10kb_bt)),
                                rep("Unbalanced", length(F1data_gm12878_10kb)))
all_rfperfs$Predictor <- rep(c("Distance", "OC", "OP"), 168)
all_rfperfs$Resampling <- rep(c(rep("None", 3),
                            rep("ROS", 3),
                            rep("RUS", 3),
...
ggplot(all_rfperfs_F1, aes(x=Predictor, y=F1, fill=Testing)) +
     #geom_line(size=2, position=position_dodge(0.5))+
     #geom_point(size=5, position=position_dodge(0.5))+
