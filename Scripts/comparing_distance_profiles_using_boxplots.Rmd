...
all_dist_gm12878$y <- factor(all_dist_gm12878$y, levels=c("Yes","No"))
...
all_dist_gm12878_10kb <- ggplot(all_dist_gm12878[which(all_dist_gm12878$Resolution=="10 kb"),], aes(x = y, y = value, fill = y)) +
  ...
  stat_compare_means(comparisons = list(c("No", "Yes")), 
                     ...
                     label = "p.signif") +
  #facet_grid(~ variable) +
  xlab("Transcription Factor Binding Site") +
  ylab("Log2 Distance") +
  #ylim(5,25) +
  scale_fill_discrete(name="Genomic bin overlapped w/ flanked TAD boundary") +
  #guides(color=FALSE) +
  theme_minimal() +
  theme_bw() +
...
annotate_figure(figure, 
                #bottom = text_grob("Transcription Factor Binding Site", color = "black", size = 20),
...
all_dist_K562$y <- factor(all_dist_K562$y, levels=c("Yes", "No"))
