...
all_propimp_gm12878$Annotation <- factor(all_propimp_gm12878$Annotation, levels=c("HM", "CRE", "TFBS"))
all_propimp_gm12878$Resolution <- factor(all_propimp_gm12878$Resolution, levels=c("5 kb", "10 kb", "25 kb", "50 kb", "100 kb"))
...
all_propimp_k562$Annotation <- ifelse(all_propimp_k562$Feature %in% chromstates, "CRE",
                                      ifelse(all_propimp_k562$Feature %in% histones, "HM", "TFBS"))
all_propimp_k562$Annotation <- factor(all_propimp_k562$Annotation, levels=c("HM", "CRE", "TFBS"))
all_propimp_k562$Resolution <- factor(all_propimp_k562$Resolution, levels=c("5 kb", "10 kb", "25 kb", "50 kb", "100 kb"))


all_propimp_k562_sum <- all_propimp_k562 %>%
  dplyr::group_by(Resolution, Annotation) %>%
  dplyr::summarise(Performance = mean(Importance, na.rm = TRUE),
                   PerformanceSD = sd(Importance, na.rm = TRUE))

ggplot(all_propimp_k562_sum, aes(fill=Annotation, y=Performance, x=Resolution)) + 
  geom_bar( stat="identity", position="fill") +
  xlab("Resolution") +
  ylab("Proportion of Importance") +
  scale_fill_discrete(name="Genomic Class")+
