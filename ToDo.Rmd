...
+ add standardization part to question 4 table
+ question 6: which variable selection technique?
+ perform scaling with transformation



## 10/01/18

* read up on random forest 
   - investigate how to get final decision tree
   - how variable importance is calculated
+ write about answers not questions; make headers as answers not questions
+ investigate how many iterations are necessary for under sampling
   - plot it
* investigate svm & neural networks & naiive bayes

## 10/03/18

* pose additional question, does doing multiple iterations of sampling improve model performance
* can we prove that the performance (f1 score & variables selected) is stable over multiple iterations

## 10/05/18

* does random forest sufffer on imbalanced data?
* use maximum instead of mean when determining cutoff of iterations
* find out if there is a convergence algorthim to decide point of convergence
   + look at 0.001 or 0.0001 for now
* look at AIC/BIC for potential cutoff
* run anova test to determine if two models are different at different iterations
* look into using c-index as performance metric
* create heatmap of variable importance from different models

## 11/09/18

*stick with elastic net (compare with all others)
